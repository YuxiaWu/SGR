...
input_format = 'dialogpt_goal_separated'
...
MINI_BATCH_SIZE = min((len(device_ids) << 3) if cuda else 8, BATCH_SIZE)
print('Mini batch size for training: {}'.format(MINI_BATCH_SIZE))
BATCH_SIZE_EVAL = min((len(device_ids) << 4) if cuda else 16, BATCH_SIZE << 1)
...
train = TextDataset.create_data(f'resources/train.{input_format}', tokenizer_or_transformer_model=transformer_model, split=(1,), shuffle=True)
val = TextDataset.create_data(f'resources/val.{input_format}', tokenizer_or_transformer_model=transformer_model, split=(1,), training=False, shuffle=False)
test = TextDataset.create_data(f'resources/test.{input_format}', tokenizer_or_transformer_model=transformer_model, split=(1,), training=False, shuffle=False)
...
dataloader_train = DataLoader(train, batch_size=MINI_BATCH_SIZE, shuffle=True, num_workers=workers, pin_memory=True, drop_last=False, collate_fn=collator_train)
