import torch
from generic_utils import get_empty_cuda_devices
from collections import defaultdict
from tqdm import tqdm
from generic_utils import current_time
from base_model import BaseModule
from json_utils import save
import os
...
class CustomPaddingTensorCollator():
    def __init__(self, key2pad_id={}, first={}):
...
def main_loop(model, dataloader, epochs=1, batch_iters=1,
    ...
    for epoch in range(epochs):
                ...
                for id_, result in results.items():
                    ...
                    for sample in result:
                        ...
                        if joint_correct:
                            scores['joint'] += 1
                total += curr_total
                acc = {'A' + target[:acc_nchars].upper(): BaseModule.ratio(scores[target], total) for target in scores if target in model.main_losses}
        ...
        if train and len(dataloader) % batch_iters:
            ...
            model.zero_grad()
        if print_all_acc:
            print({'ACC_' + target.upper(): BaseModule.ratio(scores[target], total) for target in scores})
    if save_results:
        os.makedirs('prediction', exist_ok=True)
        save(all_results, os.path.join('prediction', model_name + '_' + current_time().replace('/', '-') + '_val_acc-' + ' '.join([f'{k}:{v}' for k, v in acc.items()]) + '_val_loss-' + model.print_loss() + '.json'))
    if not train and save_best:
        joint_acc = BaseModule.ratio(scores['joint'], total)
...
def loop(model, dataloader, epochs=1, batch_iters=1, clear_cache_every=100, train=True, cuda=True, calc_loss=True, calc_acc=True, save_best=True, acc_thresh=0, model_name='MODEL', acc_nchars=1, save_results=False, print_all_acc=True):
    model.train(train)
    if train:
        print('TRAINING...')
        main_loop(model, dataloader, epochs=epochs, batch_iters=batch_iters, clear_cache_every=clear_cache_every,
                  train=train, cuda=cuda, calc_loss=calc_loss, calc_acc=calc_acc, save_best=save_best, acc_thresh=acc_thresh, model_name=model_name, acc_nchars=acc_nchars, save_results=save_results, print_all_acc=print_all_acc)
    else:
        print('EVALUATING...')
        with torch.no_grad():
            main_loop(model, dataloader, epochs=epochs, batch_iters=batch_iters, clear_cache_every=clear_cache_every,
