...
def evaluate(args, device, dataset, act_predict_model, SGR_model,tokenizer, pad_value):
    ...
    run_name = "online_test_task_1225_domain_2"
    ...
    turn_result = []
    result = []
    result_domain =   {'food': [], 'hotel': [],
				'nightlife':[], 'shopping mall':[], 'sightseeing':[] }
    

    SR_domain = {'food': np.array([0.0, 0.0, 0.0]), 'hotel': np.array([0.0, 0.0, 0.0]),
				'nightlife':np.array([0.0, 0.0, 0.0]), 'shopping mall': np.array([0.0, 0.0, 0.0]), 'sightseeing':np.array([0.0, 0.0, 0.0]) }

    SR_task_num_domain =   {'food': [], 'hotel': [],
				'nightlife':[], 'shopping mall':[], 'sightseeing':[] }
    
    with open(os.path.join(args.data_path,'uid_domain.json'), "r", encoding="utf-8") as f:
        uid_domain = json.load(f)  
    task_num = 0
    task_num_domain =   {'food': 0, 'hotel': 0,
    ...
    for user_idx in list(range(user_size)):  #user_size
        ...
        domain = uid_domain[str(uid)]
        print('domain: ', domain)
        print('task_num of this user: ', test_env.task_num)
        ...
        for task_idx in list(range(test_env.task_num)):
            ...
            for t in count():  # user  dialog
                with torch.no_grad():   
                    ...
                    data_file = 'test_online_task_1225_domain.action_prediction'
                        ...
                        if done:
                            if recommend_success:  # recommend successfully
                                ...
                                if t < 5:
                                    ...
                                    SR10 += 1
                                    SR15 += 1
                                    SR_domain[domain][1]+=1
                                    SR_domain[domain][2]+=1
                                else:
                                    SR15 += 1
                                    SR_domain[domain][2]+=1
                            AvgT += t+1
                            ...
                            task_num_domain[domain]+=1
                            result_domain[domain].append(SR)


                            print('SR_domain: ', SR_domain)
                            print('task_num_domain: ', task_num_domain)

                            SR_task_num_domain[domain] = [SR_domain[domain]/task_num_domain[domain]]
                            print('SR/task_num_domain: ', SR_task_num_domain)
                            break #  break action
    ...
    print('SR_all_mean: ',SR_all)
    print('task_num: ', task_num)

    SR5_sum = np.sum(np.array([item[0] for item in result]))
    SR10_sum = np.sum(np.array([item[1] for item in result]))
    SR15_sum = np.sum(np.array([item[2] for item in result]))
    AvgT_sum = np.sum(np.array([item[3] for item in result]))
    SR_all = [SR5_sum, SR10_sum, SR15_sum, AvgT_sum]
    print('SR_all_sum: ', SR_all)
    print('task_num: ', task_num)

    print('result_domain: ', result_domain)
    for domain, result_i in result_domain.items():
        print('domain: ', domain)
        SR5_mean = np.mean(np.array([item[0] for item in result_i]))
        SR10_mean = np.mean(np.array([item[1] for item in result_i]))
        SR15_mean = np.mean(np.array([item[2] for item in result_i]))
        AvgT_mean = np.mean(np.array([item[3] for item in result_i]))
        SR_all = [SR5_mean, SR10_mean, SR15_mean, AvgT_mean]
        print('SR_all: ',SR_all)
