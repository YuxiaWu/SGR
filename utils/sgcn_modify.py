...
from sgcn_utils import calculate_auc, setup_features
...
from sgcn_utils import structured_negative_sampling
...
class SignedGraphConvolutionalNetwork(torch.nn.Module):
    ...
    def __init__(self, device, args, node_count):
        ...
        self.feature_dim = self.args.hidden_dim
        self.setup_layers()
    ...
    def setup_layers(self):
        ...
        self.positive_base_aggregator = SignedSAGEConvolutionBase(self.feature_dim*2,
                                                                  self.neurons[0]).to(self.device)

        self.negative_base_aggregator = SignedSAGEConvolutionBase(self.feature_dim*2,
                                                                  self.neurons[0]).to(self.device)
    ...
    def forward(self, positive_edges, negative_edges, target, X):        
        """
        ...
        self.h_pos.append(torch.tanh(self.positive_base_aggregator(X, positive_edges)))
        self.h_neg.append(torch.tanh(self.negative_base_aggregator(X, negative_edges)))
        for i in range(1, self.layers):
...
class SignedGCNTrainer(object):
    ...
    def save_model(self):
        ...
        self.train_z.to_csv(os.path.join(self.args.embedding_path,'embeddings.csv'), index=None)
        ...
        self.regression_params.to_csv(os.path.join(self.args.regression_weights_path, 'regression_weights.csv'), index=None)
